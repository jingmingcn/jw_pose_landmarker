...
img_no_voice = cv2.imread('config/image/no_camera.jpg', cv2.IMREAD_COLOR)
...
def preprocess_face(face):
    ...
    face = cv2.resize(face, (64, 64))  # Resize to the required size
    face = face.astype("float") / 255.0  # Normalize pixel values
    face = img_to_array(face)
    face = np.expand_dims(face, axis=0)
    return face

def cv2PutText(img, text, postion, textColor=(0, 255, 0), textSize=30):
    if (isinstance(img,np.ndarray)):
        img = Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))

    draw = ImageDraw.Draw(img)
    font = ImageFont.truetype("font/simsun.ttc", textSize, encoding="utf-8")
    draw.text(postion, text, textColor, font=font)
...
class MyApp(QMainWindow, Ui_MainWindow, threading.Thread):
    ...
    def __init__(self,chunk=1024, rate=16000):
        ...
        self.recording_location = 'record/'
    ...
    def win_1(self):
        if self.hd_camera != -1:
            ...
            # self.cap1 = cv2.VideoCapture(self.hd_camera)
            self.cap1 = cv2.VideoCapture("elder_dance.avi")
            ...
            while self._win1_running:
                    ...
                    if ret:
                        ...
                        blank_image = draw_landmarks_on_image(blank_image, detection_result)
                        self.eye_image.updateImage(annotated_image)
                        self.original2_image.updateImage(blank_image)

                        visualized_mask = None
                        if detection_result.segmentation_masks is not None:
                            rgb = cv2.cvtColor(detection_result.segmentation_masks[0].numpy_view(), cv2.COLOR_GRAY2RGB)
                            rgb = rgb * 255
                            rgb = rgb.astype(np.uint8)
                            visualized_mask = rgb
                        else:
                            black = np.zeros(frame.shape, dtype=np.uint8)
                            visualized_mask = black

                        self.voice_image.updateImage(visualized_mask)

                            # ff.write(str(id) + "\n" + np.array2string(shape) + "\n")
    ...
    def updatemessage(self):
        ...
        self.statusbar.showMessage('当前样本编号：' + self.lineEdit.text() + '   保存位置：' + self.save_path + '   录制时间：' + self.recording_time)
    ...
    def f_p(self):
        ...
        if self.record_judge == True:
            ...
            self.voice_image.updateImage(img_no_voice)
    def about(self):
        QMessageBox.about(self, "面部表型特征提取系统",
                          "<p>山东省精神卫生中心</p>")
